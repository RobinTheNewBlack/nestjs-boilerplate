...
export class SalesTransactionItemController {
  constructor(
    private readonly salesTransactionItemService: SalesTransactionItemService,
  ) {}
  ...
  @ApiResponse({
    status: 201,
    description: 'Sales transaction item created successfully',
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Sales transaction item updated successfully',
  })
  @ApiResponse({ status: 400, description: 'Validation failed' })
  @ApiResponse({ status: 404, description: 'Sales transaction item not found' })
  update(@Param('id') id: string, @Body() data: UpdateSalesTransactionItemDto) {
    return this.salesTransactionItemService.updateSalesTransactionItem(
      id,
      data,
    );
  }
  ...
  @ApiResponse({
    status: 204,
    description: 'Sales transaction item deleted successfully',
  })
  @ApiResponse({ status: 404, description: 'Sales transaction item not found' })
